...
export const QuickActions = ({
  ...
  setLoadingRefreshYield: Dispatch<SetStateAction<boolean>>
            ...
            <PopoverItem>
              <div
                ...
                onClick={async () => {
                  ...
                  setLoadingRefreshYield(true)

                  try {
                    const response = await fetch(
                      `/api/refreshYieldInstruction/`,
                      {
                        method: 'POST',
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                          mint: stakedTokenData.stakeEntry!.parsed.originalMint.toString(),
                          userPubkey: wallet.publicKey?.toString(),
                          stakePoolKey: stakePool.pubkey.toString(),
                        })
                      }
                    )
                    ...
                    if (json.type === 'error') {
                      throw json.message
                    } else if (json.type === 'ignored') {
                      notify({
                        message: `Yield rate (${json.yieldRate} $HAY/day) already up-to-date`,
                        type: 'success',
                      })
                    }
                  }
                    ...
                    notify({
                      message: `Failed to update the yield rate`,
                      type: 'error'
                    })
                  }
                  finally {
                    setLoadingRefreshYield(false)
                    setLoading(false)
                  }
                }}
              >
